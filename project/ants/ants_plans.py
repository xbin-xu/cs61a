...
def make_test_assault_plan(ants_impl=None):
    ...
    return (
        AssaultPlan().add_wave(ants_impl.Bee, 3, 2, 1).add_wave(ants_impl.Bee, 3, 3, 1)
    )
...
def make_easy_assault_plan(ants_impl=None):
    ...
    for time in range(3, 16, 2):
        plan.add_wave(
            ants_impl.Bee, 3, time, 1
        )  # Adding 1 bee of health 3 at timestamp TIME
...
def make_normal_assault_plan(ants_impl=None):
    ...
    for time in range(
        3, 16, 2
    ):  # Adding 2 bees (1 bee for time = 3, 5) of health 3 at timestamp 3, 5, 7, 9, 11, 13, 15
        if time == 3 or time == 5:
    ...
    for time in range(
        12, 15, 2
    ):  # Adding 1 Wasp (health 3) and 1 Ninja (health 2) at time 12, 14
        plan.add_wave(ants_impl.Wasp, 3, time, 1)
        plan.add_wave(ants_impl.Wasp, 2, time, 1)

    for time in range(
        13, 19, 2
    ):  # Adding 2 Wasp (health 3) + 2 Ninja (health 3) at time 13, 15, 17
        plan.add_wave(ants_impl.Wasp, 3, time, 2)
        plan.add_wave(ants_impl.Wasp, 3, time, 2)

    for time in range(
        16, 23, 2
    ):  # Adding 2 bees of health 5 at timestamp 16, 18, 20, 22
        plan.add_wave(ants_impl.Bee, 5, time, 2)

    for time in range(
        24, 31, 2
    ):  # Adding 2 bees of health 6 at timestamp 24, 26, 28, 30
...
def make_hard_assault_plan(ants_impl=None):
    ...
    for time in range(
        3, 9, 2
    ):  # Adding 2 bees (1 bee for time = 3) of health 3 at timestamp 3, 5, 7
        if time == 3:
            # Change number of bees to 1
            plan.add_wave(ants_impl.Bee, 3, time, 1)
        else:
    ...
    for time in range(
        10, 15, 2
    ):  # Adding 2 Wasp (health 3) and 1 Ninja (health 3) at time 10, 12, 14
        plan.add_wave(ants_impl.Wasp, 3, time, 2)
        plan.add_wave(ants_impl.Wasp, 3, time, 1)

    for time in range(
        13, 19, 2
    ):  # Adding 3 Wasp (health 3) + 3 Ninja (health 3) at time 13, 15, 17
        plan.add_wave(ants_impl.Wasp, 3, time, 3)
        plan.add_wave(ants_impl.Wasp, 3, time, 3)

    for time in range(
        16, 23, 2
    ):  # Adding 3 bees of health 5 at timestamp 16, 18, 20, 22
        plan.add_wave(ants_impl.Bee, 5, time, 3)

    for time in range(
        24, 31, 2
    ):  # Adding 3 bees of health 6 at timestamp 24, 26, 28, 30
        plan.add_wave(ants_impl.Bee, 6, time, 3)

    for time in range(
        20, 30, 2
    ):  # Adding 2 Ninja of health 3 at time 20, 22, 24, 26, 28
        plan.add_wave(ants_impl.Wasp, 3, time, 2)
...
def make_extra_hard_assault_plan(ants_impl=None):
    ...
    for time in range(
        10, 15, 2
    ):  # Adding 2 Wasp (health 3) and 2 Ninja (health 3) at time 10, 12, 14
        plan.add_wave(ants_impl.Wasp, 3, time, 2)
        plan.add_wave(ants_impl.Wasp, 3, time, 2)

    for time in range(
        13, 19, 2
    ):  # Adding 3 Wasp (health 4) + 3 Ninja (health 3) at time 13, 15, 17
    ...
    for time in range(
        16, 25, 2
    ):  # Adding 3 bees of health 6 at timestamp 16, 18, 20, 22, 24
        plan.add_wave(ants_impl.Bee, 6, time, 3)
...
def create_game_state():
    ...
    parser.add_argument(
        '-w', '--water', action='store_true', help='loads a full layout with water'
    )
    parser.add_argument(
        '--food', type=int, help='number of food to start with when testing', default=2
    )
