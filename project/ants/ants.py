...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while idx < self.lower_bound:
            if place is None or place.is_hive:
                return None
            place = place.entrance
            idx += 1
        while idx <= self.upper_bound:
            if place is None or place.is_hive:
                return None
            if place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        amount += self.damage if self.health <= amount else 0
        for bee in list(self.place.bees):
            bee.reduce_health(amount)
        super().reduce_health(min(self.health, amount))
        # END Problem 5
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt eats a random Bee from its place for each `chewing_turns` turns."""
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        return self.ant_contained is None and other.is_container is False
        # END Problem 8a
...
class TankAnt(BodyguardAnt):
    """TankAnt provides protection to other Ants."""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower can be placed on Water."""
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        # return 0
        return self.damage - distance * 0.25 - self.insects_shot * (1 / 16)
        # END Problem Optional 2
